...
struct No * criaNo(int chave, const char *palavra)
{
...
int main() {
    ...
    while(scanf("%s", comando) != EOF) {
        if(!strcmp(comando, "insert")) {
            ret = scanf("%s", palavra);
            for(i=0, chave=0; palavra[i]!='\0'; i++){
                chave+=(int) palavra[i];
            }
            //printf("%s: [%d] %s\n", comando, chave, palavra);
            inserir(arvore, criaNo(chave, palavra));
        } elif(!strcmp(comando, "delete")) {
            ret = scanf("%s", palavra);
            for(i=0, chave=0; palavra[i]!='\0'; i++){
                chave+=(unsigned char) palavra[i];
            }
            //printf("%s: [%d] %s\n", comando, chave, palavra);
